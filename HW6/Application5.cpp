...
int Application5::combine2One()
{
	...
	for (int i = 0; i < m_finalDisplay->yres; i++)
	{
		for (int j = 0; j < m_finalDisplay->xres; j++)
		{
			//set default color
            int index = (j + (i * m_finalDisplay->xres));
			m_finalDisplay->fbuf[index].red = 0;
			m_finalDisplay->fbuf[index].green = 0;
			m_finalDisplay->fbuf[index].blue = 0;
			m_finalDisplay->fbuf[index].alpha = 0;
			m_finalDisplay->fbuf[index].z = 0;
			
			//combine all colors together based on the filter weight
			for (int k = 0; k < AAKERNEL_SIZE; k++)
			{
				m_finalDisplay->fbuf[index].red = m_finalDisplay->fbuf[index].red + m_pDisplay[k]->fbuf[index].red * AAFilter[k][2];
				m_finalDisplay->fbuf[index].green = m_finalDisplay->fbuf[index].green + m_pDisplay[k]->fbuf[index].green * AAFilter[k][2];
				m_finalDisplay->fbuf[index].blue = m_finalDisplay->fbuf[index].blue + m_pDisplay[k]->fbuf[index].blue * AAFilter[k][2];
				m_finalDisplay->fbuf[index].alpha = m_finalDisplay->fbuf[index].alpha + m_pDisplay[k]->fbuf[index].alpha * AAFilter[k][2];
				m_finalDisplay->fbuf[index].z = m_finalDisplay->fbuf[index].z + m_pDisplay[k]->fbuf[index].z * AAFilter[k][2];
			}
